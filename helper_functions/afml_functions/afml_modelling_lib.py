...
def getTrainTimes(t1,testTimes):
    ...
    for i,j in testTimes:
        df0=trn[(trn.index>=i)&(trn.index<=j)].index # the train label indices which START inside a testTime interval [i,j]
...
class PurgedKFold(KFold): # this id called a generator class, we will generate the class BaseKFold with the adaptions written in the generator functions in the PurgedKFold generator function
    def __init__(self,n_splits=3,t1=None,pctEmbargo=0.):
        if not isinstance(t1,type(pd.Series())):
            raise ValueError('Label through dates must be a pd.Series')
        super().__init__(n_splits,shuffle=False,random_state=None) # here we initiate the base class which is the standard sklearn k-fold cross validator
        self.t1=t1
        self.pctEmbargo=pctEmbargo
    def split(self,X,y=None,groups=None):
        if (X.index==self.t1.index).sum()!=len(self.t1):
            raise ValueError('X and ThruDateValues must have the same index') # just checking that our t1 labels is the same as our datasupplie in X
        indices=np.arange(X.shape[0])
        mbrg=int(X.shape[0]*self.pctEmbargo) # this is the number of indices we will skip after the test set as a resdult of the embargo
        test_starts=[(i[0],i[-1]+1) for i in np.array_split(np.arange(X.shape[0]),self.n_splits)]
        for i,j in test_starts:
            t0=self.t1.index[i] # represent the label indices THESE ARE TIME INDICES
            test_indices=indices[i:j] # this is defined by the iteration of start and ends we are on THESE ARE INTEGER INDICES
            # the searchsorted function will return the integer indices, not the time indices
            maxT1Idx=self.t1.index.searchsorted(self.t1[test_indices].max()) # we obtain the index of the first time after the latest time within the test set
            train_indices=self.t1.index.searchsorted(self.t1[self.t1<=t0].index) # PURGING (left of test set) - this gets all the indices that occur where the label end time is before the label start times of the test set
            if maxT1Idx<X.shape[0]: # check if the test set isn't at the end of the data
                train_indices=np.concatenate((train_indices,indices[maxT1Idx+mbrg:])) # EMBARGO (right of test set) - concatenates the integer train indices to the left of the test set with the integer, embargoed train indices to right of test set
...
def cvScore(clf,X,y,sample_weight,scoring='neg_log_loss',t1=None,cv=None,cvGen=None,pctEmbargo=None): # this is a fix to some bugs in the sklearn KFold class
    ...
    score=[]
    for train,test in cvGen.split(X=X): # the split function will return a new train and test set each time it cycles through the function (remember it saves its progress in the loop each time it loops)
...
def featImpMDI(fit,featNames):
    ...
    df0=df0.replace(0,np.nan) # we ensure that its highlighted that a 0 importance just means we didn't select it at all
    imp=pd.concat({'mean':df0.mean(),'std':df0.std()/(df0.shape[0]**0.5)},axis=1) # we obtain the standard error on the mean here
...
def featImpMDA(clf,X,y,cv,sample_weight,t1,pctEmbargo,scoring='neg_log_loss'):
    ...
    for i,(train,test) in enumerate(cvGen.split(X=X)): # the split function will return a new train and test set each time it cycles through the function (remember it saves its progress in the loop each time it loops)
    ...
    imp=pd.concat({'mean':imp.mean(),'std':imp.std()/(imp.shape[0]**0.5)},axis=1) # gives mean importance and std error on each feature
...
def get_eVec(dot,varThres):
    ...
    eVec=pd.DataFrame(eVec,index=dot.index,columns=eVal.index) # vector now written with principle component names in columns
...
def orthoFeats(dfX,varThres=0.95): # dfX is our usual dataframe of features
...
def getTestData(n_features=40,n_informative=10,n_redundant=10,n_samples=1000):
    ...
    df0=pd.date_range(periods=n_samples,freq=pd.tseries.offsets.BDay(),end=pd.datetime.today()) # business day frequency, finishing today
...
def featImportance(trnsX,cont,n_estimators=1000,cv=10,max_samples=1.,numThreads=24,pctEmbargo=0,scoring='accuracy',method='MDI',fitter='BaggedTree',minWLeaf=0.,**kwargs): # min leaf is where below this weight, the node becomes a leaf (so 0 means must have only 1 sample left to be a leaf)
    ...
    if fitter=='BaggedTree':
        ...
        from sklearn.ensemble import BaggingClassifier
        clf=DecisionTreeClassifier(criterion='entropy',max_features=int(1),class_weight='balanced',min_weight_fraction_leaf=minWLeaf)
        ...
        clf=RandomForestClassifier(n_estimators=n_estimators,max_features=int(1),max_samples=max_samples,class_weight='balanced',min_weight_fraction_leaf=minWLeaf,n_jobs=n_jobs)
...
def testFunc(n_features=40,n_informative=10,n_redundant=10,n_estimators=1000,n_samples=10000,cv=10):
    ...
    jobs,out=(dict(zip(dict0,i)) for i in product(*dict0.values())),[] # here we look at all the combinations of arguments (product(*dict0.values())), then zip each of these with each argument label in the dictionary, then convert each entry in the list to a dictionary
    ...
    for job in jobs:
        ...
        df0['type']=[i[0] for i in df0.index] # first letter of feature gives whether it is important, redundant, noise
        df0=df0.groupby('type')['mean'].sum().to_dict() # total importance of each type of feature - we should now have one value for each of these three types
...
def parallelized_featImp(datasets,n_estimators=1000,cv=10,max_samples=1.,numThreads=24,pctEmbargo=0,scoring='accuracy',method='MDI',fitter='BaggedTree',minWLeaf=0.):
    ...
    for i in range(len(datasets)):
        imp,oob,oos=featImportance(datasets[i][0],datasets[i][1],n_estimators=n_estimators,cv=cv,max_samples=max_samples,numThreads=numThreads,pctEmbargo=pctEmbargo,scoring=scoring,method=method,minWLeaf=minWLeaf,fitter=fitter)
...
class MyPipeline(Pipeline):
    def fit(self,X,y,sample_weight=None,**fit_params):
        if sample_weight is not None:
            fit_params[self.steps[-1][0]+'__sample_weight']=sample_weight # so if we try to use the fit() function with a sample_weight parameter, it will just assign the sample weight to the final model's sample weight param name in fit_params and will then use it in the pipeline's fit function as usual. (see below for explanation)
...
def clfHyperFit(feat,lbl,t1,pipe_clf,param_grid,cv=3,bagging=[0,None,1.],rndSearchIter=0,n_jobs=-1,pctEmbargo=0,**fit_params):
    ...
    # ***
    gs=gs.fit(feat,lbl,**fit_params).best_estimator_ # gives the model with the best hyperparameters selected (remember usually we input the hyperparameters when we initialize a fitter, so this is the same principle) - note: this gs will still be a pipeline
    # fit final model with selected hyperparameters
    if bagging[1]>0: # of course if max_samples is 0, we are not taking any samples in each bag, so we are not doing any bagging
        gs=BaggingClassifier(base_estimator=MyPipeline(gs.steps), # here we ensure we use the updated pipeline class so that the sample_weight param can be automatically passed through the base model's fit function
                             n_estimators=int(bagging[0]),max_samples=float(bagging[1]),n_jobs=n_jobs)
        gs=gs.fit(feat,lbl,sample_weight=fit_params[gs.base_estimator.steps[-1][0]+'__sample_weight']) # here we need to get the sample_weight from fit_params
        gs=Pipeline(['bag',gs]) # create the final pipeline which is just our bagging classifier (again we see here final step is an estimator)
    return gs


class logUniform_gen(rv_continuous): # inherited from scipy
    def _cdf(self,x):
