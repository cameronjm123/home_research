...
def getDollarSampleIndices(dollar_interval, dollar_traded):
    cumsum = 0
    indices = []
...
def getDailyVolClose(close, span0=100):
    df0 = close.index.searchsorted(close.index-pd.Timedelta(days=1))
    df0 = df0[df0 > 0]
    df0 = pd.Series(close.index[df0], index=close.index[close.shape[0] - df0.shape[0]:]).to_frame() # all the above does is creates 2 columns with adjacent dates in each column
    df0['close'] = close.loc[df0.index] # returns
    df0['vol'] = df0['close'].ewm(span=span0).std() # compute std of ewm of price
    return df0


def getDailyVolRet(close, span0=100):
    df0 = close.index.searchsorted(close.index-pd.Timedelta(days=1))
    df0 = df0[df0 > 0]
    df0 = pd.Series(close.index[df0], index=close.index[close.shape[0] - df0.shape[0]:]) # all the above does is creates 2 columns with adjacent dates in each column
...
def sym_cusum(raw_data, h): # raw data here is the returns if we are using a boundary which is of the unit of returns (std of returns is unit of returns) - remember we are checking if returns are accumulating in some way
...
def applyPtSlOnT1(close, events, ptSl, molecule): # (PtSl means profit taking and stop loss)
    '''
...
def getEvents(close, tEvents, ptSl, trgt, minRet, numThreads, t1 = False, side = None):
    ...
    df0 = applyPtSlOnT1(close, events, ptSl, events.index)
...
def getBins(events, close):
    ...
    out['ret'] = px.loc[events['t1'].values].values / px.loc[events.index] - 1 # takes return from the initial indexed time to the barrier hit
...
def mpNumCoEvents(closeIdx,t1,molecule):
    ...
    count = pd.Series(0, index=closeIdx[iloc[0]:iloc[1]+1]) # just ensures we use the time indices that occur AFTER the closest time indices to t1 start and finish (t_i,0 to the very last time entry in t1)
...
def getIndMatrix(barIx, t1):
    ...
    '''
    indM = pd.DataFrame(0, index=barIx, columns=range(t1.shape[0])) # we construct a matrix with rows indexed by the raw data, and columns equal to the number of triple barriers
    # Now we want to set the element to 1 if the time on the row index is contained in the triple barrier interval t0:t1
    for i, (t0,t1) in enumerate(t1.iteritems()): # here we go through each triple barrier interval, remember the columns are indexed by numerate indices, hence the i here to check which column we're 'filling in'
        indM.loc[t0:t1,i] = 1 # here we are looking at column i (which corresponds to the particular triple barrier interval) and assigning 1 to any of the raw data indices between the particular triple barrier intervals 2 times, [t0,t1]
    return indM
...
def getAvgUniqueness(indM):
    ...
    '''
    c = indM.sum(axis=1) # calculates number of concurrent labels at t (see  notes) so for each row we sum across that row, so count the 1 entries in the matrix (so returns a table with rows equal to the raw data number of rows)
    u = indM.div(c, axis=0) # calculates the uniqueness for each triple barrier interval, giving yet another matrix with the same dimensions as indM
    avgU = u[u>0].mean() # we only calculate the mean across t when for t intervals overlapping with the particular triple barrier interval (as we want average uniqueness across just that interval)
    return avgU


def seqBootstrap(indM, sLength=None): #sLength allows us to have bootstrap samples of any size if desired
    if sLength is None:
    ...
    while len(phi) < sLength: # outer loop is taking the samples until we reach our desired size
        ...
        for i in indM: # inner loop is going through all the columns of indM, so we are looking at each observation (triple barrier interval) and calculating the average uniqueness across t
            ...
            indM_ = indM[phi + [i]] # reduce indM to only the columns which include the target vatriables sampled and the current target variable for which the average uniqueness is being calculated
            avgU.loc[i] = getAvgUniqueness(indM_).iloc[-1] # we take the last index as we see above that the variable i under consideration has been added on as the last column (and will therefore show up as the last index)
        ...
        phi += [np.random.choice(indM.columns, p=prob)] # adds to the list the next sample taken at random according to the probabilities
    return phi
...
def plotMinFFD(series,thres=1e-2):
    from statsmodels.tsa.stattools import adfuller
    out=pd.DataFrame(columns=['adfStat','pVal','lags','nObs','95% conf','corr'])
    for d in np.linspace(0,1,11):
        df1=np.log(series[['close']]).resample('1D').last() # resamples series to daily instead of whatever it was before and uses log prices
        df1=np.log(series[['close']])
        df1=series[['close']]
        df2=fracDiff_FFD(df1,d,thres) # our new fractionally differentiated series using fixed window method
        corr=np.corrcoef(df1.loc[df2.index,'close'],df2['close'])[0,1] # correlation between differenced series and original series - we want this to be high as this indicates the series are similar with regards to information, therefore we have not lost a lot of memory for high correlation coefficients
